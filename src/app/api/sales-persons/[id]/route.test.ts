...
describe('/api/sales-persons/[id]', () => {
  ...
  describe('GET /api/sales-persons/[id]', () => {
    it('営業担当者の詳細を正常に取得できる', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/sales-persons/1'
      );
    ...
    it('存在しないIDで404エラーが返る', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/sales-persons/999'
      );
    ...
    it('不正なID形式で400エラーが返る', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/sales-persons/invalid'
      );
  ...
  describe('DELETE /api/sales-persons/[id]', () => {
    it('関連データがない場合は物理削除される', async () => {
      ...
      mockPrismaClient.salesPerson.delete.mockResolvedValue(
        existingSalesPerson
      );
    ...
    it('関連データがある場合は論理削除される', async () => {
      ...
      mockPrismaClient.dailyReport.findFirst.mockResolvedValue(
        relatedReports[0]
      ); // 関連する日報あり
