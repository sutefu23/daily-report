...
class PerformanceAnalyzer {
  ...
  constructor(
    resultsDir: string = './performance-tests/results',
    outputDir: string = './performance-tests/reports'
  ) {
   ...
   * 閾値のチェック
   */
  private checkThresholds(summary: K6Summary): {
    passed: boolean;
    failed: string[];
  } {
    const failed: string[] = [];
  ...
  private generateRecommendations(testResults: TestResult[]): string[] {
    ...
    // 平均応答時間のチェック
    const avgResponseTimes = testResults.map(
      (t) => t.performance.avgResponseTime
    );
    const overallAvg =
    ...
    if (overallAvg > 2000) {
      recommendations.push(
        '平均応答時間が2秒を超えています。データベースクエリの最適化やキャッシュの導入を検討してください。'
      );
    }
    ...
    if (maxP95 > 3000) {
      recommendations.push(
        '95パーセンタイルの応答時間が3秒を超えています。重い処理の非同期化を検討してください。'
      );
    }
    ...
    if (maxErrorRate > 0.01) {
      recommendations.push(
        'エラー率が1%を超えているテストがあります。エラーログを確認し、原因を調査してください。'
      );
    }
    ...
    testResults.forEach((test) => {
      if (!test.thresholdsPassed) {
        recommendations.push(
          `${test.testName}のテストが失敗しています。失敗した閾値: ${test.failedThresholds.join(', ')}`
        );
      }

      if (
        test.testName.includes('concurrent') &&
        test.performance.errorRate > 0.005
      ) {
        recommendations.push(
          '同時アクセステストでエラーが発生しています。接続プール設定やリソース制限を確認してください。'
        );
      }

      if (
        test.testName.includes('create') &&
        test.performance.p95ResponseTime > 2000
      ) {
        recommendations.push(
          '作成処理のパフォーマンスが低下しています。トランザクション処理やバリデーション処理を見直してください。'
        );
      }
    });
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'すべてのパフォーマンステストが基準を満たしています。'
      );
      recommendations.push(
        '定期的なパフォーマンステストの実行を継続してください。'
      );
    }
