...
test.describe('アクセシビリティテスト', () => {
  ...
  test.describe('ダッシュボードのアクセシビリティ', () => {
    test('一般ユーザーダッシュボードのアクセシビリティチェック', async ({
      page,
    }) => {
  ...
  test.describe('日報機能のアクセシビリティ', () => {
    ...
    test('日報テーブルのアクセシビリティ', async ({ page }) => {
      ...
      for (let i = 0; i < headerCount; i++) {
        ...
        expect(
          scope === 'col' || (await header.getAttribute('role'))
        ).toBeTruthy();
      }
    });
    ...
    test('コメントフォームのアクセシビリティ', async ({ page }) => {
      ...
      const ariaLabelledBy =
        await commentTextArea.getAttribute('aria-labelledby');
      const associatedLabel = page.locator('label[for*="comment"]');

      // ラベルが適切に関連付けられていることを確認
      expect(
        ariaLabel || ariaLabelledBy || (await associatedLabel.count()) > 0
      ).toBeTruthy();
    });
  });
