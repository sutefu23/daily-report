...
export class MasterManagementPage {
  ...
  constructor(page: Page, pageType: 'customers' | 'sales-persons') {
    ...
    this.newButton = page.locator(
      'button:has-text("新規登録"), button:has-text("新規")'
    );
    this.searchField = page.locator(
      'input[name="search"], input[placeholder*="検索"]'
    );
  ...
  async clickEditRow(identifier: string): Promise<void> {
    const editButton = this.page.locator(
      `tr:has-text("${identifier}") button:has-text("編集")`
    );
  ...
  async clickDeleteRow(identifier: string): Promise<void> {
    const deleteButton = this.page.locator(
      `tr:has-text("${identifier}") button:has-text("削除")`
    );
...
export class CustomersPage extends MasterManagementPage {
  ...
  async expectCustomerInList(
    companyName: string,
    contactPerson: string
  ): Promise<void> {
    const row = this.tableRows
      .filter({ hasText: companyName })
      .filter({ hasText: contactPerson });
    await expect(row).toBeVisible();
  }
  ...
  async addCustomer(customerData: {
    ...
    await this.page
      ...
      .fill(customerData.companyName);
    await this.page
      .locator('input[name="contactPerson"]')
      .fill(customerData.contactPerson);
  ...
  async editCustomer(
    ...
    if (newData.companyName) {
      await this.page
        .locator('input[name="companyName"]')
        .fill(newData.companyName);
    }
    if (newData.contactPerson) {
      await this.page
        .locator('input[name="contactPerson"]')
        .fill(newData.contactPerson);
    }
    if (newData.phone) {
  ...
  async deleteCustomer(companyName: string): Promise<void> {
    ...
    const confirmButton = this.page.locator(
      'button:has-text("削除"), button:has-text("はい")'
    );
...
export class SalesPersonsPage extends MasterManagementPage {
  ...
  async addSalesPerson(salesPersonData: {
    ...
    await this.page
      ...
      .fill(salesPersonData.password);
    await this.page
      .locator('input[name="department"]')
      .fill(salesPersonData.department);

    const managerCheckbox = this.page.locator(
      'input[name="isManager"], input[type="checkbox"]'
    );
