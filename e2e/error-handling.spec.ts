...
test.describe('エラーハンドリング', () => {
  test.describe('バリデーションエラー', () => {
    ...
    test('日報作成フォームのバリデーション', async ({ page }) => {
      ...
      // 文字数制限を超える入力
      await reportFormPage.fillProblem(
        VALIDATION_TEST_DATA.MAX_LENGTH_EXCEEDED.problem
      );
      await reportFormPage.fillPlan(
        VALIDATION_TEST_DATA.MAX_LENGTH_EXCEEDED.plan
      );
    ...
    test('訪問記録のバリデーション', async ({ page }) => {
      ...
      const customerError = page.locator(
        '[data-testid="customer-error"], .customer-error'
      );
      const contentError = page.locator(
        '[data-testid="visitContent-error"], .visit-content-error'
      );
      ...
      await reportFormPage.addVisitRecord(
        'ABC商事',
        '10:00',
        VALIDATION_TEST_DATA.MAX_LENGTH_EXCEEDED.visitContent
      );
    ...
    test('コメント投稿のバリデーション', async ({ page }) => {
      ...
      // 文字数制限を超えるコメント
      await reportDetailPage.postComment(
        VALIDATION_TEST_DATA.MAX_LENGTH_EXCEEDED.comment
      );
    ...
    test('顧客マスタのバリデーション', async ({ page }) => {
      ...
      // バリデーションエラーが表示されることを確認
      const companyNameError = page.locator(
        '[data-testid="companyName-error"], .field-error'
      );
      const contactPersonError = page.locator(
        '[data-testid="contactPerson-error"], .field-error'
      );
  ...
  test.describe('ネットワークエラー', () => {
    test('ログイン時のネットワークエラー', async ({ page }) => {
      ...
      const errorMessage = page.locator(
        'text=接続エラー, text=ネットワークエラー, text=サーバーに接続できません'
      );
    ...
    test('日報保存時のネットワークエラー', async ({ page }) => {
      ...
      // エラーメッセージが表示されることを確認
      const errorMessage = page.locator(
        'text=保存に失敗, text=エラーが発生, text=ネットワークエラー'
      );
      await expect(errorMessage.first()).toBeVisible();
    });
    ...
    test('データ取得時のネットワークエラー', async ({ page }) => {
      ...
      const errorMessage = page.locator(
        'text=データの取得に失敗, text=エラーが発生, text=再試行'
      );
      await expect(errorMessage.first()).toBeVisible();
    });
  });
  ...
  test.describe('サーバーエラー', () => {
    ...
    test('404エラーのハンドリング', async ({ page }) => {
      ...
      const errorMessage = page.locator(
        'text=見つかりません, text=存在しません, text=404'
      );
      await expect(errorMessage.first()).toBeVisible();
    });
    ...
    test('403権限エラーのハンドリング', async ({ page }) => {
      ...
      const errorMessage = page.locator(
        'text=権限がありません, text=403, text=アクセスが拒否'
      );
      await expect(errorMessage.first()).toBeVisible();
    });
  });
  ...
  test.describe('UIエラーケース', () => {
    test('必須フィールドの視覚的フィードバック', async ({ page }) => {
      ...
      await expect(reportFormPage.problemField).toHaveClass(
        /error|invalid|border-red/
      );
    });
    ...
    test('セッション切れ時の適切な処理', async ({ page }) => {
      ...
      const authError = page.locator(
        'text=セッションが切れました, text=再ログインしてください'
      );
  ...
  test.describe('データ整合性エラー', () => {
    test('同時編集による競合エラー', async ({ page }) => {
      ...
      const conflictMessage = page.locator(
        'text=他のユーザー, text=更新されています, text=競合'
      );
