...
test.describe('権限制御テスト', () => {
  test.describe('一般ユーザーの権限', () => {
    ...
    test('ナビゲーションメニュー - 管理者限定メニューが非表示', async ({
      page,
    }) => {
      const dashboardPage = new DashboardPage(page);
      ...
      const customerManagementLink = page.locator(
        'a[href="/customers"], text=顧客管理'
      );
      const salesPersonManagementLink = page.locator(
        'a[href="/sales-persons"], text=営業担当者管理'
      );
  ...
  test.describe('クロスユーザーアクセステスト', () => {
    test('ユーザーAで作成した日報をユーザーBが編集しようとする', async ({
      page,
    }) => {
