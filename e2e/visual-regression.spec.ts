...
test.describe('ビジュアル回帰テスト', () => {
  ...
  test.describe('ダッシュボード', () => {
    test('一般ユーザーダッシュボードのスクリーンショット', async ({ page }) => {
      ...
      // 個別カードのスクリーンショット
      await expect(dashboardPage.reportsCard).toHaveScreenshot(
        'dashboard-reports-card.png'
      );
      await expect(dashboardPage.visitsCard).toHaveScreenshot(
        'dashboard-visits-card.png'
      );
    });
  ...
  test.describe('日報機能', () => {
    test('日報一覧画面のスクリーンショット', async ({ page }) => {
      ...
      // 検索フォーム部分のスクリーンショット
      await expect(reportsPage.searchForm).toHaveScreenshot(
        'reports-search-form.png'
      );

      // テーブル部分のスクリーンショット
      await expect(reportsPage.reportTable).toHaveScreenshot(
        'reports-table.png'
      );
    });
    ...
    test('日報詳細画面のスクリーンショット', async ({ page }) => {
      ...
      // 問題セクションのスクリーンショット
      await expect(reportDetailPage.problemSection).toHaveScreenshot(
        'report-problem-section.png'
      );

      // 訪問記録セクションのスクリーンショット
      await expect(reportDetailPage.visitRecordsSection).toHaveScreenshot(
        'report-visits-section.png'
      );
    });
    ...
    test('コメント機能のスクリーンショット', async ({ page }) => {
      ...
      // コメントフォームのスクリーンショット
      await expect(reportDetailPage.commentForm).toHaveScreenshot(
        'comment-form.png'
      );

      // コメント投稿後のスクリーンショット
      await reportDetailPage.postComment('テストコメントです。');
      await expect(reportDetailPage.commentsSection).toHaveScreenshot(
        'comments-section-with-comment.png'
      );
    });
  });
