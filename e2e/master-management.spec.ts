...
test.describe('マスタ管理機能', () => {
  test.describe('顧客マスタ管理', () => {
    ...
    test('顧客マスタ一覧表示', async ({ page }) => {
      ...
      for (const customer of TEST_CUSTOMERS) {
        await customersPage.expectCustomerInList(
    ...
    test('顧客削除', async ({ page }) => {
      ...
      const deletedRow = page.locator(
        `tr:has-text("${testCustomer.companyName}")`
      );
  ...
  test.describe('営業担当者マスタ管理', () => {
    ...
    test('営業担当者新規登録', async ({ page }) => {
      ...
      await salesPersonsPage.expectSalesPersonInList(
        newSalesPerson.name,
    ...
    test('営業担当者情報編集', async ({ page }) => {
      ...
      const managerIndicator = page.locator(
        `tr:has-text("${updatedData.name}"):has-text("管理者")`
      );
    ...
    test('営業担当者削除', async ({ page }) => {
      ...
      await salesPersonsPage.expectSalesPersonInList(
        testSalesPerson.name,
        testSalesPerson.department
      );
    ...
    test('営業担当者登録時のバリデーション', async ({ page }) => {
      ...
      const nameError = page.locator(
        '[data-testid="name-error"], .field-error'
      );
      const emailError = page.locator(
        '[data-testid="email-error"], .field-error'
      );
      const passwordError = page.locator(
        '[data-testid="password-error"], .field-error'
      );
  ...
  test('一般ユーザーのマスタ管理アクセス制限', async ({ page }) => {
    ...
    const errorMessage = page.locator(
      'text=権限がありません, text=403, text=Forbidden'
    );
    await expect(errorMessage.first()).toBeVisible();
